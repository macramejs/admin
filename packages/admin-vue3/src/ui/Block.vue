<template>
    <draggable
        v-model="repeatables"
        :component-data="{ class: 'flex flex-col' }"
    >
        <template #item="{ element }">
            <div class="relative px-8 -mx-8 bg-white">
                <div
                ...
                <Disclosure v-slot="{ open }">
                    <DisclosureButton
                        class="flex items-center justify-between w-full py-4 text-left  focus:outline-none"
                    >
                        <div>
                            {{ element.title }}
                        </div>
                    ...
                    <div
                        v-if="open"
                        class="mb-4 -mx-8 border-b border-gray-300"
                    ></div>

                    <DisclosurePanel> Foo </DisclosurePanel>
                    <div
                        v-if="open"
                        class="mt-4 -mx-8 border-b border-gray-300"
                    ></div>
                </Disclosure>
                <button
...
export default defineComponent({
    ...
    // },
    setup() {
        const repeatables = ref([
            {
                title: 'Foo bar baz',
                body: 'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Officia reiciendis nobis ipsum consectetur tempora est quam quis sapiente dolor illo eos illum molestiae, nesciunt fugit dolorum ratione inventore architecto tempore.',
            },
            {
